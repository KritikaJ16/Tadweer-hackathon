...
import {
  ...
  MessageCircle, Send, RefreshCw, Settings,
} from "lucide-react";
...
const STR = {
  en: {
    ...
    off: "Off",
    // AI business coach
    agentName: "Pulse · Business Coach",
    agentTagline: "Your AI guide to starting a business in Al Qua'a",
    agentOpen: "Open the business coach",
    agentGreeting: "Marhaba! I'm Pulse, your business coach. Ask me anything about starting or growing a business in Al Qua'a — costs, customers, licensing, marketing, or any opportunity you found here. Where would you like to begin?",
  ...
  ar: {
    ...
    agentName: "Pulse · مستشار الأعمال",
    agentTagline: "دليلك الذكي لبدء مشروع في القوع",
    agentOpen: "افتح مستشار الأعمال",
    agentGreeting: "مرحبًا! أنا Pulse، مستشارك في الأعمال. اسألني أي شيء عن بدء أو تنمية مشروع في القوع — التكاليف، العملاء، التراخيص، التسويق، أو أي فرصة وجدتها هنا. من أين تحب أن نبدأ؟",
...
/* AI CONFIG + API CALL                                                    */
/* ---------------------------------------------------------------------- */
// Read configuration from Vite env vars (see .env.example).
const PULSE_MODEL = import.meta.env.VITE_ANTHROPIC_MODEL || "claude-sonnet-4-6";
const PULSE_PROXY_URL = import.meta.env.VITE_API_PROXY_URL || "";
const PULSE_API_KEY = import.meta.env.VITE_ANTHROPIC_API_KEY || "";

// Returns { ok, data } | { ok:false, reason }.
// Prefers a server-side proxy (keeps the key secret). Falls back to a direct
// browser call only for local development — never ship a real key to production.
async function callPulse(system, messages) {
  const payload = { model: PULSE_MODEL, max_tokens: 1000, system, messages };

  if (PULSE_PROXY_URL) {
    const r = await fetch(PULSE_PROXY_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    if (!r.ok) return { ok: false, reason: "proxy" };
    return { ok: true, data: await r.json() };
  }

  if (!PULSE_API_KEY) return { ok: false, reason: "nokey" };
...
const COACH_SYS = (lang) => `You are Pulse, a warm, practical AI business coach built into "Desert Pulse" — a community platform that helps entrepreneurs in Al Qua'a, a small rural town in the Al Ain region of Abu Dhabi, UAE, start businesses their neighbours actually need.
...
function BusinessCoach({ lang }) {
  ...
  const send = async (text) => {
    ...
    setLoading(true);
    try {
      const result = await callPulse(
        COACH_SYS(lang),
        next.map(m => ({ role: m.role, content: m.content }))
      );
      if (!result.ok) throw new Error(result.reason || "request failed");
      ...
      {open && (
        <div className="dp-coach-panel" role="dialog" aria-label={t("agentName", lang)}>
          ...
          <div className="dp-coach-body" ref={bodyRef}>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`dp-coach-msg ${m.role}`} dir="auto">{m.role === "assistant" ? renderRich(m.content) : m.content}</div>
            ))}
            {loading && (
...
export default function App() {
  ...
  const speak = (key, text) => {
    ...
    // re-query voices at call time (the cached list is often empty on first paint)
    const voices = synth.getVoices && synth.getVoices().length ? synth.getVoices() : voicesRef.current;
    const match = (voices || []).find(v => v.lang && v.lang.toLowerCase().replace("_", "-").startsWith(target));
    const start = () => {
      const u = new SpeechSynthesisUtterance(text);
      u.lang = target === "ar" ? "ar-SA" : "en-US";
      if (match) u.voice = match;       // only force a voice that matches the language
      u.rate = 0.97;
    ...
    // Chrome drops an utterance spoken in the same tick as cancel(); defer a little
    setTimeout(start, 60);
  };
  ...
  const submitResponse = (answers) => {
    const entries = Object.entries(answers).filter(([, v]) => v && v.trim());
    if (!entries.length) return;
    setResponses(r => [{ id: Date.now(), time: Date.now(), answers: Object.fromEntries(entries) }, ...r]);
  };
  ...
  return (
    <div className={`dp-app ${isRTL ? "dp-rtl" : ""} ${hc ? "dp-hc" : ""}`} dir={isRTL ? "rtl" : "ltr"} lang={lang}>
      ...
      {/* PAGES */}
      {page === "landing" && <LandingPage go={go} lang={lang} extraResponses={responses.length} />}
      {page === "survey" && (
...
/* LANDING                                                                 */
/* ---------------------------------------------------------------------- */
function LandingPage({ go, lang, extraResponses = 0 }) {
  const title = t("heroTitle", lang);
  const statValue = (s) => {
    if (s.key !== "communityResponses") return s.value;
    const base = parseInt(String(s.value).replace(/,/g, ""), 10) || 0;
    return (base + extraResponses).toLocaleString("en-US");
  };
  return (
    <>
